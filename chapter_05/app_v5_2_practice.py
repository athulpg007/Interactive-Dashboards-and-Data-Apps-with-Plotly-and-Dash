...
app.layout = html.Div([
	...
	dbc.Row([
		...
		dbc.Col([
			dcc.Dropdown(id='gini_country_dropdown', options=[{'label': country, 'value': country}
...
def plot_gini_country_barchart(country):
	if not country:
		raise PreventUpdate
	df = poverty[poverty['Country Name'].eq(country)].sort_values(gini).dropna(subset=[gini])
